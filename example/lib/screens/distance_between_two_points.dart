...
import 'package:google_maps_distance/google_maps_distance.dart';
...
class _DistanceBetweenTwoPointsState extends State<_DistanceBetweenTwoPoints> {
  ...
  Route get shortestRoute => _directions!.shortestRoute;
  ...
  void _add(LatLng point) {
    ...
    if (_origin == null) {
      _origin = Point(lat: point.latitude, lng: point.longitude);
    } else {
      _destination = Point(lat: point.latitude, lng: point.longitude);
    }
  ...
  Future<void> _setupRoutes(Point p1, Point p2) async {
    _polylines = [];
